...
## Script using PyMOL for alignment and RMSD calculation of enzyme-complex structures from docking experiments. 
...
import pymol 
from pymol import cmd #as the PyMOL GUI is not opened, a message will appear saying it is entering library mode



# Step 2: Load the .pdb files for all protein structures

import glob #glob module finds pathnames matching a specified pattern according to the rules of the Unix shell, in the below case it uses the special character *
...
def align_all_to_targetactKR(target,mobile_selection='c. B and n. CA+C+N+O',target_selection='c. B and n. CA+C+N+O'): 
    """
    Align function for the actKR (chain B) backbone atoms of HADDOCK structures onto a target. The mobile and target selections are defined but the target must be specified when using the function - in this case it is M14.

    It uses cmd.align() which is the PyMOL align command.

    When alignment is complete, a completion statement is printed to the terminal. 
    """   
...
def calculate_rmsd(target): 
    ...
    Firstly, the xyz coordinates of the target actACP are obtained and the xyz coordinates for the actACP of each HADDOCK structure in the list are obtained. This is using the get_xyz() function.
    
    The RMSD is calculated using the RMSD equation which is written using numpy arrays. Each RMSD value for each HADDOCK structure is appended to a list (rmsd_list) which is then ordered by size and printed to the terminal. This allows the identification of the favourable HADDOCK structures. 
